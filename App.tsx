...
import { EXPERIENCE, PROJECTS, SOCIALS, BIO, NAV_LINKS, CERTIFICATIONS, RECRUITER_CONTENT } from './constants';
...
const Section = ({ id, children, className = "" }: { id: string; children?: React.ReactNode; className?: string }) => {
  return (
    <motion.section 
      ...
      className={`min-h-screen flex flex-col justify-center snap-start py-16 lg:py-0 px-6 md:px-12 lg:px-0 ${className}`}
    >
  ...
  return (
    <div className="h-screen w-screen bg-neutral-50 dark:bg-neutral-950 text-neutral-600 dark:text-neutral-400 selection:bg-teal-300 selection:text-teal-900 overflow-hidden relative flex transition-colors duration-500">
      ...
      <div className="w-full h-full max-w-screen-xl mx-auto flex relative z-10">
          ...
          <header className="hidden lg:flex lg:w-1/2 h-full flex-col justify-between py-20 px-12 lg:px-24 relative z-10">
            <div>
          ...
          {/* RIGHT COLUMN: Scrollable Content with Snapping */}
          <main 
            ref={scrollContainerRef}
            className="flex-1 h-full overflow-y-auto snap-y snap-mandatory scroll-smooth scrollbar-hide relative z-10 lg:px-12"
          >
            {/* Mobile Header (Visible only on small screens) */}
            <div className="lg:hidden pt-16 px-6 mb-8 flex flex-col items-start">
              <div className="mb-6 relative group w-max">
                <div className="absolute -inset-1 bg-gradient-to-r from-teal-500 to-cyan-500 rounded-full blur opacity-25"></div>
                <img 
                  src="/profile.jpg" 
                  alt="Badrish MS" 
                  className="relative w-24 h-24 rounded-full object-cover border-2 border-neutral-200 dark:border-neutral-900"
                />
              </div>
              <h1 className="text-4xl font-bold tracking-tight text-neutral-900 dark:text-neutral-100">Badrish MS</h1>
              <h2 className="mt-2 text-lg font-medium text-neutral-800 dark:text-neutral-200">Data Engineer</h2>
              <div className="mt-6 flex gap-4">
                 {SOCIALS.map((social) => {
                  const Icon = Icons[social.icon as keyof typeof Icons] || Icons.ExternalLink;
                  return (
                    <a key={social.platform} href={social.url} className="text-neutral-400 hover:text-neutral-900 dark:hover:text-white">
                      <Icon size={20} />
                    </a>
                  );
                })}
